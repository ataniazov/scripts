...
connect() {
	...
	if is_android ; then
		...
		ROUTE_CONFIG='\
			ifconfig tun'$CLIENT_TUN' up; \
			ifconfig tun'$CLIENT_TUN' '$CLIENT_TUN_IP' pointopoint '$SERVER_TUN_IP' netmask '$NETMASK'; \
			ip route replace default via '$SERVER_TUN_IP'; \
			ip route add 192.168.20.1 via 30.10.21.1; \
			ip rule add to 192.168.20.1 dev '$(get_interface)'; \
			ip rule add from 192.168.20.1 dev '$(get_interface)'; \
			ip route add 192.168.100.10 via 30.10.21.1; \
			ip rule add to 192.168.100.10 dev '$(get_interface)'; \
			ip rule add from 192.168.100.10 dev '$(get_interface)'; \
			ip route add 192.168.100.11 via 30.10.21.1; \
			ip rule add to 192.168.100.11 dev '$(get_interface)'; \
			ip rule add from 192.168.100.11 dev '$(get_interface)'; \'

			#ip route add default via 30.10.21.1 dev enp3s1 table kou;
...
while true; do
	...
	connect
	defaultdns
	echo "waiting 5 second to retry..."
	sleep 5
done
