...
connect() {
	...
	if [ "$SERVER_SCRIPT" == "local" ]; then
		...
		REMOTE_COMMAND='ifconfig tun'$SERVER_TUN' '$SERVER_TUN_IP' pointopoint '$CLIENT_TUN_IP' netmask '$NETMASK'; \
			...
			\
			INTERNET=$(ip route get 8.8.8.8 | head -n 1 | awk '"'"'{print $5}'"'"' | tr -d '"'"'\n'"'"') ; \
			echo "[SERVER] internet interface: $INTERNET" ; \
			\
			echo 1 > /proc/sys/net/ipv4/ip_forward; \
			\
			RULE1="-i '$TUN' -o $INTERNET -j ACCEPT" ; \
			RULE2="-i $INTERNET -o '$TUN' -m state --state ESTABLISHED,RELATED -j ACCEPT" ; \
			RULE3="-o $INTERNET -j MASQUERADE" ; \
			\
			`iptables -C FORWARD $RULE1 2&>1 > /dev/null` || iptables -A FORWARD $RULE1 ; \
			`iptables -C FORWARD $RULE2 2&>1 > /dev/null` || iptables -A FORWARD $RULE2 ; \
			`iptables -t nat -C POSTROUTING $RULE3  2&>1 > /dev/null` || iptables -t nat -A POSTROUTING $RULE3 ; \
			\
